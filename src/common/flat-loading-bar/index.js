export default class {
  constructor(element, transitionDelay = 1000) {
    ...
    this.element.style.setProperty(
      "transition",
      `width ${this.transitionDelay}ms ease-in-out, 
            opacity ${this.transitionDelay}ms ease-in-out`,
    );
    setInterval(() => {
      if (this.queueDelay > 50) this.queueDelay -= 50;
    }, 50);
  }

  setPercentage(percentage, customDelay = this.transitionDelay) {
    this.postToQueue(
      () => this.element.style.setProperty("width", `${percentage}%`),
      customDelay,
    );
  }
  ...
  increasePercentage(percentage, customDelay = this.transitionDelay) {
    const current = Number(
      this.element.style.getPropertyValue("width").replace("%", ""),
      customDelay,
    );
  ...
  disappear(customDelay = this.transitionDelay) {
    ...
    this.postToQueue(
      () =>
        this.element.style.setProperty(
          "transition",
          `${this.transitionDelay}ms ease-in-out`,
        ),
      customDelay,
    );
  }
  ...
  static getOnly(document, transitionDelay = 1000) {
    const element = Array.from(
      document.getElementsByClassName("flat-loading-bar"),
    )[0];
