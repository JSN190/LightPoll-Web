  ...
  <main class="container">
    <div class="columns">
          ...
          <form id="main-poll-form">
            ...
            <div class="card-content">
              <textarea
                ...
                class="main-poll-form-option 140char"
                rows="1"
                maxlength="140"
                :key="index - 1"
            ...
            <footer class="card-footer">
              <div>
                <a
                  ...
                  v-on:click="sendPollAndRetrieveId"
                >
                  Create Poll
                </a>
              </div>
              <div id="main-poll-checkbox-group">
                ...
                <label class="checkbox">
                  <input type="checkbox" v-model="enforceUnique" /> Prevent
                  duplicate votes
                </label>
              </div>
            </footer>
      ...
      <div class="column is-one-third">
        ...
        <div class="br-50"></div>
        <p>
          LightPoll allows you to instantaneosly create and share online polls
          without registration.
        </p>
        <div class="br-25"></div>
        <p>
          It is a free and open source software project released under the MIT
          License.
        </p>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    flushAndPushErrors: function() {
      let errors = new Set(),
        uniqueOptions = [];
      const emptyOptions = this.options.reduce(
        (acc, e) => (String(e).trim().length < 1 ? acc + 1 : acc),
        0,
      );
      const nonEmptyOptions = this.options.filter(
        e => String(e).trim().length >= 1,
      );

      if (!this.name) errors.add("This poll needs a name.");
      if (this.name && !validator.isAscii(this.name))
        errors.add("Unacceptable characters in name.");
      if (this.name && this.name.length > 140)
        errors.add("The poll name is too long.");
      if (nonEmptyOptions.length < 2)
        errors.add("This poll needs at least two options.");
      if (emptyOptions === this.options.length)
        errors.add("All options are empty.");
      for (let option of this.options) {
        option = String(option);
        if (option.length >= 1 && !validator.isAscii(option))
          errors.add("Unacceptable characters in one or more options.");
        if (option.length > 140) errors.add("One or more options is too long.");
        if (option.length >= 1 && uniqueOptions.includes(option))
