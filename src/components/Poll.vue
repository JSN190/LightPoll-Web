        ...
        <section class="connection">
          <div class="connection-badge">
            <div>
              <i
                class="fas fa-circle"
                :style="`color: ${badgeColours[this.status]}`"
              ></i>
            </div>
            <div>STREAM {{ this.status.toUpperCase() }}</div>
            <div
              class="connection-badge-reconnect"
              v-if="status === 'Disconnected' || status === 'Error'"
              v-on:click="streamPoll"
            >
              reconnect
            </div>
          </div>
          ...
          <div v-if="status === 'Connected' || status === 'Connecting'">
            New votes {{ status === "Connected" ? "are" : "will be" }} instantly
            delivered to your browser in real-time.
          </div>
          ...
          <div v-if="status === 'Error'">
            An unexpected error has occured during communication with the
            server. Maybe you could try and connect again.
          </div>
        </section>
          ...
          <table>
            ...
            <tr>
              ...
              <td>
                {{
                  totalVotes >= 1
                    ? formatedTimestampes.latestVote
                    : "No votes yet."
                }}
              </td>
            </tr>
            ...
            <tr>
              <td>Unique Votes:</td>
              <td>
                <div v-if="voters !== 'unlimited'">
                  Enforced&nbsp;<i class="fas fa-lock"></i>
                </div>
                <div v-if="voters === 'unlimited'">
                  Not Enforced&nbsp;<i class="fas fa-lock-open"></i>
                </div>
              </td>
            </tr>
            <tr>
        ...
        <section class="poll-info" v-if="name && belongsToRoute">
          ...
          <div class="link-set">
            <input
              type="text"
              class="sharing-url"
              v-on:click="shareEventHandler"
              readonly="true"
              v-on:keypress="shareEventHandler"
              :value="`${url}/poll/${$route.params.id}`"
              spellcheck="false"
            />
...
export default {
      ...
      return {
        created: this.created
          ? this.created
              .toLocaleString()
              .replace(/:([0-9]){2}$/, "")
              .replace(/[0-9]{4},/, match => match.substring(0, 4))
          : null,
  ...
  methods: {
    ...
    copyShareUrl: function(event) {
      navigator.clipboard
        .writeText(`${this.url}/poll/${this.$route.params.id}`)
        .then(() => (event.target.innerText = "copied"))
        .catch(() => (event.target.innerText = "failed"));
      event.target.parentElement
        .getElementsByClassName("sharing-url")[0]
        .select();
    },
    ...
    streamPoll: async function() {
      try {
        ...
        this.stream = new EventSource(
          `${this.api}/poll/${this.$route.params.id}/stream`,
        );
