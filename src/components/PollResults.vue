<template>
  <div id="main-poll-card" class="card">
    ...
    <div class="card-content">
      ...
      >
        ...
        <div class="option-votes-bar">
          <div
            class="option-votes-bar-share"
            :data-width="`${votePercentage(option.votes) * 100}%`"
            :style="
              `background-color: ${option.votes ? option.colour : '#F0F0F0'};`
            "
          >
            <div class="options-votes-bar-percent">
              {{ (votePercentage(option.votes) * 100).toFixed(0) }}%
            </div>
          </div>
        </div>
        <div class="option-votes">
          {{
            option.votes > 0
              ? option.votes +
                (option.votes > 1 ? " people" : " person") +
                " voted for this option."
              : "Nobody has voted for this option yet."
          }}
        </div>
      </div>
    </div>
    <div class="card-footer">
      <div>
        <router-link class="footer-container" :to="`/poll/${$route.params.id}/`"
          ><i class="fas fa-arrow-left">&nbsp;</i
          ><span>Back to voting</span></router-link
        >
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    totalVotes() {
      return this.localOptions.reduce(
  ...
  methods: {
    ...
    assignOptionColours: function(reset) {
      ...
      this.localOptions.forEach((option, index) => {
        do {
          colours.add(randomColor({ luminosity: "bright" }));
        } while (colours.size < index + 1);
        if (!option.hasOwnProperty("colour") || reset)
          option.colour = Array.from(colours)[index];
      });
    },
    animateVoteBars: function(delay = 100) {
      setTimeout(() => {
        const bars = Array.from(
          document.getElementsByClassName("option-votes-bar-share"),
        );
        bars.forEach((bar, index) => {
          setTimeout(() => {
            const label = bar.getElementsByClassName(
            ...
            label.style.setProperty(
              ...
              `${
                tinyColor(
                  bar.style.getPropertyValue("background-color"),
                ).isLight()
                  ? "black"
                  : "white"
              }`,
